...
export class AuditSDK extends EventEmitter {
  ...
  private reportSystem: UnifiedReportSystem;
  
  constructor(config: SDKConfig = {}) {
    super();
    this.config = this.mergeConfig(config);
    this.configManager = new ConfigManager();
    this.reportSystem = new UnifiedReportSystem();
  ...
  private async generateReports(
    ...
    const reportData: ReportData = {
      ...
      metadata: {
        ...
        version: this.sdk.getVersion(),
        duration: pipelineResult.summary.totalDuration || 0,
        sitemapUrl: this.sitemapUrl,
        environment: process.env.NODE_ENV || 'development'
      },
      config: options
    ...
    const reportOptions: ReportOptions = {
      ...
      includePa11yIssues: options.usePa11y,
      summaryOnly: false,
      prettyPrint: true,
      branding: {
        company: 'AuditMySite SDK',
        footer: `Generated by AuditMySite SDK v${this.sdk.getVersion()}`
      }
    };
    ...
    for (const format of options.formats) {
      try {
        ...
        const generated = await this.sdk['reportSystem'].generateReport(format, reportData, reportOptions);
